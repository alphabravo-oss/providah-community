...
All existing AWS, DigitalOcean and Hetzner discovery calls `inventory.Collect` from the public Apache-2.0 module [`ab-provider-modules`](https://github.com/alphabravo-oss/ab-provider-modules), pinned at `v0.2.3`. No sibling checkout or private credentials are needed to build.
