...
class CloudSmokeConfiguration(unittest.TestCase):
    def test_offline_check_and_private_credentials(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            result = subprocess.run(command + ["--url", "http://localhost:8761", "--login-file", str(Path(directory) / "login.json"), "--output", str(Path(directory) / "report.json")], capture_output=True, text=True)
            self.assertEqual(result.returncode, 0, result.stderr)
            result = subprocess.run(command + ["--url", "https://example.com"], capture_output=True, text=True)
