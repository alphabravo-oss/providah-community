...
export function CreateResourceButton({ org }: { org: Organization }) {
  ...
  const policy=useQuery({queryKey:["resource-policy",org.id],queryFn:()=>api.getResourcePolicy({organizationId:org.id}),enabled:open});
  ...
  return <><button className="primary" onClick={()=>{setSelection(undefined);setConnection(undefined);setDraft(undefined);create.reset();setOpen(true);}}>Add resource</button>
    <Modal open={open} onOpenChange={setOpen} title={title} description="Review the exact configuration before confirming creation.">
      ...
      {!selection ? <ActionForm key="resource-type" fields={[
        ...
        <ErrorNote error={policy.error}/><p className="notice">{!policy.data?"Loading approval policy…":policy.data.approvalActions.includes("create")?"Another authorized person must approve after you confirm.":"Confirmation is the final gate: creation queues directly after you submit."}</p>
        <ActionForm fields={[{name:"reason",label:sshKey?"Import reason":"Creation reason",type:"textarea",schema:z.string().min(3).max(500)},{name:"confirm",label:sshKey?"Confirm the reviewed key name":"Confirm the reviewed server name",schema:z.literal(draft.name)}]} onSubmit={v=>create.mutateAsync(v)} submitLabel="Confirm creation" pending={create.isPending||policy.isPending||policy.isError} error={create.error}/>
